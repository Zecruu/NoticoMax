...
export function Sidebar({
  ...
  const canonicalDefaultFolder = getCanonicalDefaultNotesFolder(personalFolders);
  ...
  const handleRename = async (clientId: string) => {
    ...
    const folder = personalFolders.find((candidate) => candidate.clientId === clientId);
        ...
        {sectionsOpen.developer && (
          <nav className="space-y-1">
            ...
            >
              ...
              <span className="flex-1 text-left">Codex Skills</span>
            </button>

            <button
              onClick={() => {
                onFolderChange(null);
                onViewChange?.("envvars");
                onTagChange?.(null);
              }}
              className={cn(
                "flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors",
